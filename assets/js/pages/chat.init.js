...
(function () {
    var dummyUserImage = "https://img.themesbrand.com/judia/users/user-dummy-img.jpg";
    var dummyMultiUserImage = "https://img.themesbrand.com/judia/users/multi-user.jpg";
    ...
    function toggleSelected() {
        ...
        Array.from(document.querySelectorAll(".chat-user-list li a")).forEach(function (item) {
            item.addEventListener("click", function (event) {
                ...
                // chat user list link active or unactive
                var chatUserList = document.querySelectorAll(".chat-user-list li.active");
                ...
                // remove msg count
                this.parentNode.querySelector(".unread-msg-user .chat-user-msg-count")?.remove();

                // remove class
                this.parentNode.querySelector(".unread-msg-user")?.classList.remove("unread-msg-user");
            });
    ...
    getJSON("chat-users-list.json", function (err, data) {
        if (err !== null) {
            ...
            users.forEach(function (userData, index) {
                ...
                var isMessageCount = userData.messagecount ? '<div class="mt-1"><span class="badge bg-danger p-0 d-flex align-items-center justify-content-center ms-auto chat-user-msg-count">' +
                ...
                document.getElementById("userList").innerHTML = document.getElementById("favUserList").innerHTML +=
                ...
                <div class="d-flex align-items-center">\
                    ...
                    <div class="flex-grow-1 overflow-hidden me-2">\
                        <h6 class="text-truncate mb-1 lh-base">' + userData.name + '</h6>\
                        <p class="text-truncate text-muted fs-xs fw-normal mb-0">' + userData.chatMsg + '</p>\
                    </div>\
                    <div class="flex-shrink-0 fs-2xs align-self-start text-end">\
                    '+ userData.lastSeen + '\
                    ' + isMessageCount + '\
                    </div>\
                </div>\
            ...
            channelsData.forEach(function (isChannel, index) {
                var isMessage = isChannel.messagecount
                    ? '<div class="flex-shrink-0 ms-2"><span class="badge bg-dark-subtle text-body rounded p-0 d-flex align-items-center justify-content-center chat-user-msg-count">' +
                    isChannel.messagecount +
                    "</span></div>"
                    : "";
                var isMessageCount = isChannel.messagecount ? '<div class="ms-auto"><span class="badge bg-dark-subtle text-body rounded p-0 d-flex align-items-center justify-content-center chat-user-msg-count">' +
                    isChannel.messagecount +
                    "</span></div>"
                    : "";
    ...
    function getNextMsgs(chatsData, i, from_id, isContinue) {
        ...
        while (chatsData[i]) {
            if (chatsData[i + 1] && chatsData[i + 1]["from_id"] == from_id) {
                msgs = getMsg(chatsData[i + 1].id, chatsData[i + 1].msg, chatsData[i + 1].has_images, chatsData[i + 1].has_files, chatsData[i + 1].has_dropDown, chatsData[i + 1].has_voting, chatsData[i + 1].has_reactEmoji, chatsData[i + 1].isIncomingCall);
                i++;
            } else {
                break;
            }
    ...
    // getMeg
    function getMsg(id, msg, has_images, has_files, has_dropDown, has_voting, has_reactEmoji, isIncomingCall) {
        ...
        if (msg != null) {
            ...
            var progessElem = '';
            has_voting[0].progess.forEach(function (element) {
                progessElem += '<div class="progress progress-xl mb-2 bg-body-secondary" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" data-bs-original-title="72%" role="progressbar" aria-label="Basic example" aria-valuenow="' + element.progressbar + '" aria-valuemin="' + element.progressbar + '" aria-valuemax="100">\
                <div class="progress-bar" style="width: '+ element.progressbar + '%">' + element.title + '</div>\
            </div>'
            });

            msgHTML +=
                '<div class="ctext-wrap-content">\
                <p class="ctext-content">'+ has_voting[0].msg + '</p>\
                <div>'+ progessElem + '</div>\
            </div>';
            ...
            msgHTML +=
                ...
                <div class="d-flex flex-wrap align-items-center gap-2">\
                    ...
                    <div>\
                        <button class="btn btn-sm btn-success" data-bs-toggle="modal" data-bs-target="#exampleModal">Answer</button>\
        ...
        msgHTML += '</div>';
        if (has_reactEmoji && has_reactEmoji.length > 0) {
            var emojiElem = '';
            has_reactEmoji.forEach(function (element) {
                emojiElem += '<a href="#!" class="badge bg-light text-body"><span>' + element.reactIcon + '</span><span>' + element.reactNum + '</span></a>'
            });
            msgHTML += '<div class="ctext-reply-emoji d-flex gap-1 flex-wrap mb-2">' + emojiElem + '</div>';
        }
        return msgHTML;
    }
        ...
        getJSONFile(jsonFileUrl, function (err, data) {
            if (err !== null) {
                ...
                chatsData.forEach(function (isChat, index) {
                    ...
                    msgHTML += getMsg(isChat.id, isChat.msg, isChat.has_images, isChat.has_files, isChat.has_dropDown, isChat.has_voting, isChat.has_reactEmoji, isChat.isIncomingCall);
                    ...
                    msgHTML += '<i class="ri-heart-fill text-danger liked-message d-none"></i>'
    ...
    if (chatForm) {
        ...
        chatForm.addEventListener("submit", function (e) {
            ...
            if (chatInputValue.length === 0 && file == null) {
    ...
    function chatSwap() {
        document.querySelectorAll("#userList li").forEach(function (item) {
            item.addEventListener("click", function () {
                ...
                if (document.getElementById(contactId).querySelector(".userprofile")) {
                    ...
                    document.querySelector(".profile-offcanvas .avatar-xl").setAttribute("src", userProfile);
                } else {
                    document.querySelector("#users-chat .user-chat-topbar .avatar-xs").setAttribute("src", dummyUserImage);
                    document.querySelector(".profile-offcanvas .avatar-xl").setAttribute("src", dummyUserImage);
                }
        ...
        document.querySelectorAll("#channelList li").forEach(function (item) {
            item.addEventListener("click", function () {
                ...
                document.querySelector(".profile-offcanvas .avatar-xl").setAttribute("src", dummyMultiUserImage);
                if (isreplyMessage == true) {
                    isreplyMessage = false;
                    document.querySelector(".replyCard").classList.remove("show");
                }
    ...
    function deleteMessage() {
        ...
        deleteItems.forEach(function (item) {
            item.addEventListener("click", function () {
                if (item.closest(".user-chat-content").querySelectorAll(".ctext-wrap").length > 1) {
                    item.closest(".chat-wrap").remove()
                } else {
                    if (item.closest(".ctext-wrap").closest(".conversation-list").parentElement.classList.contains("chat-list")) {
                        item.closest(".ctext-wrap").closest(".conversation-list").parentElement.remove();
                    }
                }
            });
        });
    ...
    function replyMessage() {
        ...
        replyMessage.forEach(function (item) {
            item.addEventListener("click", function () {
                ...
                var replyMsg = item.closest(".ctext-wrap").querySelector(".ctext-content").innerText;
    ...
    var getChatList = function (chatid, chatItems) {
        ...
        if (document.getElementById('imageInput')) {
            ...
            if (file) {
                encodeImageFileToBase64(file, function (base64String) {
        ...
        setTimeout(() => {
            if (chatItems != null) {
                var chatCon = '<li class="chat-list right" id="chat-list-' +
                    messageIds +
                    '" >\
                <div class="conversation-list">\
                ...
                if (imageUrl)
                    chatCon += '<img src="' + imageUrl + '" width="150px">';
                chatCon += '</div>\
                            <div class="dropdown align-self-start message-box-drop">\
                                <a class="dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">\
                                    <i class="bi bi-three-dots-vertical"></i>\
                    ...
                    <i class="ri-heart-fill text-danger liked-message d-none"></i>\
    ...
    setTimeout(() => {
        if (document.querySelectorAll("#users-conversation .ctext-wrap")) {
            Array.from(document.querySelectorAll("#users-conversation .ctext-wrap")).forEach((element) => {
                element.addEventListener("dblclick", function (e) {
                    if (element.nextSibling.classList.contains("d-none")) {
...
function encodeImageFileToBase64(file, callback) {
...
//         });
//         getChatList(1, "dfsdf", base64String);
//     } else {
...
function speechRecognition() {
    if ('SpeechRecognition' in window || 'webkitSpeechRecognition' in window) {
        ...
        recognition.lang = 'en-US, fr-FR, es-ES, hi-IN, gu-IN'; // Set the language for speech recognition
        ...
        startButton.addEventListener('click', () => {
            ...
            navigator.mediaDevices.getUserMedia({ audio: true })
                .then((stream) => {
                    ...
                    startButton.classList.remove("btn-ghost-success");
                    startButton.classList.add("btn-ghost-danger");
        ...
        recognition.onend = () => {
            startButton.classList.remove("btn-ghost-danger");
            startButton.classList.add("btn-ghost-success");
        };
        ...
        // If the browser doesn't support the SpeechRecognition API
...
document.getElementById('imageInput').addEventListener('change', function (e) {
    if (document.getElementById("tempImgFile")) {
        alert("Upload only single image");
    } else {
        ...
        if (file) {
            ...
            setTimeout(() => {
                if (document.querySelector(".replyCard .replymessage-block .flex-grow-1 .mb-0")) {
                    ...
                    img.setAttribute("id", "tempImgFile");
                    img.src = imageTmpUrl;
                    img.style.width = "150px";
                    img.style.height = "auto";
                    document.querySelector(".replyCard .replymessage-block .flex-grow-1 .mb-0").appendChild(img);

                    // show toggle
                    replyToggleOpen.classList.add("show");

                    // remove file and close toggle
                    replyToggleCloseNew.addEventListener("click", function () {
                        if (document.getElementById("tempImgFile")) {
                            document.getElementById("tempImgFile").remove();
                        }
                        replyToggleOpen.classList.remove("show");
                        imageInput.value = null;
                        isreplyMessage = false;
...
// 
// connect call
var joinSimpleCall = document.getElementById("joinSimpleCall");
if (joinSimpleCall) {
    joinSimpleCall.addEventListener("click", function (e) {
        e.preventDefault();
...
// disconnect call
var disconnectSimpleCall = document.getElementById("disconnectSimpleCall")
if (disconnectSimpleCall) {
    disconnectSimpleCall.addEventListener("click", function (e) {
        e.preventDefault();
