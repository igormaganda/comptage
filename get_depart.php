...
if(isset($_POST['top_pays']) ) {
    ...
    $sql = "SELECT DISTINCT code_department, name_department  FROM gestion_pays WHERE country_name = ?";
    ...
    try {
        ...
        foreach($selectedCountries as $country) {
            ...
            foreach($regions as $row) {
                $options .= "<option value='" . $row['code_department'] . "'>".($row['code_department']). " " . $row['name_department'] . "</option>";
            }
        }
    ...
    echo "<option value=''>Aucun departement  sélectionné</option>";
}
?>
