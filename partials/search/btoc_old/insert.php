			...
			if (($handle = fopen("../".$this->file_name, "r")) !== FALSE) {
				...
				while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
					...
					for ($c=0; $c < $num; $c++) {
						...
						//$requete ='INSERT INTO b2c(email, email_md5, domain, groupe_domaine, gender, title, firstname, lastname, dateofbirth, yearofbirth, AgeGroupe, Adresse_1, Adresse_2, CP, Ville, Domain, Groupe_Domaine, Last_Date_R, Last_Date_O, Last_Date_C, Pression, Activity, R) 
						$requete = 'INSERT INTO b2C(email, email_md5, groupe_domaine, domain)
                        VALUES
							("'.$items["Email"]. '", "'.$items["Email_MD5"].'", "'.$items["Groupe_Domaine"].'", "'.$items["Domain"].'")
						;';

						echo $requete . '<br /><br />';
                        //, "'.$items["Email_MD5"].'""'.$items["Date_In"].'", "'.$items["Tel_mobile"].'", "'.$items["Tel_fixe"].'", "'.$items["Gender"].'", "'.$items["Title"].'", "'.$items["FirstName"].'", "'.$items["LastName"].'", "'.$items["DateOfBirth"].'", "'.$items["YearOfBirth"].'", "'.$items["AgeGroupe"].'", "'.$items["Adresse_1"].'", "'.$items["Adresse_2"].'", "'.$items["CP"].'", "'.$items["Ville"].'", "'.$items["Domain"].'", "'.$items["Groupe_Domaine"].'", "'.$items["Last_Date_R"].'", "'.$items["Last_Date_O"].'", "'.$items["Last_Date_C"].'", "'.$items["Pression"].'", "'.$items["Activity"].'", "'.$items["R"].'"
					}
				}
				fclose($handle);
			}
