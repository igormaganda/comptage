...
$alias = $tools->removeSpecialChars(trim($_POST["domaine_name"]));
$alias = preg_replace("# {1,}#", "-", strtolower($alias));

$nom = utf8_decode(trim($_POST["domaine_name"]));
$nom = str_replace("'", "''", $nom);

$requete = "INSERT INTO gestion_domaine(nom, alias) VALUES ('" . $nom . "', '" . $alias . "')";

if ($bdd->executeSimpleExecRequete($requete)) {
    header('Location: ./domaines.php?u=ok');
} else {
    header('Location: ./domaines.php?u=no');
}
