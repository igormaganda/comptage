...
$alias = $tools->removeSpecialChars(trim($_POST["prog_name"]));
$alias = preg_replace("# {1,}#", "-", strtolower($alias));

$nom = trim($_POST["prog_name"]);
$nom = str_replace("'", "''", $nom);

$requete = "INSERT INTO gestion_programme(nom, alias, partenaire) VALUES ('$nom', '$alias', '" . $_POST["partenaire"] . "')";


if ($bdd->executeSimpleExecRequete($requete)) {
    header('Location: ./programmes.php?u=ok');
} else {
    header('Location: ./programmes.php?u=no');
}
